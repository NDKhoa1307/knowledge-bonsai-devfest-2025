...
export const ChatBox = ({ username, onTreeGenerated }: ChatBoxProps) => {
  ...
  const handleSendMessage = async () => {
    ...
    if (useMockMode) {
      ...
      console.log('📄 [MOCK DATA] Simulated request:', JSON.stringify({
        username: username,
      ...
      try {
        ...
        // If tree data is returned, call the callback
        if ((response.tree || response.data?.tree) && onTreeGenerated) {
          console.log('🌳 [ChatBox] Tree data received, triggering onTreeGenerated callback');
          onTreeGenerated(response.tree || response.data.tree);
        }
